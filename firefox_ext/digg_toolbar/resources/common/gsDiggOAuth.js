...
GlaxDigg.Digg.OAuth = function() {
  ...
  OAuth.setProperties(OAuth, // utility functions
  {
    percentEncode: function percentEncode(s) {
      ...
      if (GlaxDigg.Util.UtilityService.isArray(s)) {
        var e = "";
      ...
      // Source: http://developer.mozilla.org/en/docs/Core_JavaScript_1.5_
      //         Reference:Global_Functions:encodeURIComponent
      s = s.replace(/\!/g, "%21");
      s = s.replace(/\*/g, "%2A");
      s = s.replace(/\'/g, "%27");
      s = s.replace(/\(/g, "%28");
    ...
    getParameterList: function getParameterList(parameters) {
      ...
      if (GlaxDigg.Util.UtilityService.isArray(parameters)) {
        return parameters;
      }
      var list = [];
    ...
    completeRequest: function completeRequest(message, accessor) {
      ...
      if (map.oauth_consumer_key == null) {
        OAuth.setParameter(
          message, "oauth_consumer_key", accessor.consumerKey || "");
      }
  ...
  OAuth.declareClass(OAuth, "SignatureMethod",
  ...
  OAuth.setProperties(OAuth.SignatureMethod, // class members
  {
    ...
    /** Create a subclass of OAuth.SignatureMethod, with the given getSignature
        function. */
      ...
      for (var p = 0; p < list.length; ++p) {
        ...
        if (nvp[0] != "oauth_signature") {
          sortable.push([ OAuth.percentEncode(nvp[0])
                        + " " // because it comes before any character that can
  ...
  OAuth.SignatureMethod.registerMethodClass(["HMAC-SHA1", "HMAC-SHA1-Accessor"],
    OAuth.SignatureMethod.makeSubclass(
      function getSignature(baseString) {
        ...
        var b64pad  = "="; /* base-64 pad char. "=" for strict RFC compliance */
        ...
        function binb2str(bin) {
          ...
          for(var i = 0; i < bin.length * 32; i += chrsz)
            str +=
              String.fromCharCode((bin[i>>5] >>> (32 - chrsz - i%32)) & mask);
          return str;
        }
