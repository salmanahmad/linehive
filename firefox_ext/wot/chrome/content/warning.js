...
var wot_warning =
{
	init: function()
	{
		try {
			...
			dump("wot_warning.init: failed with " + e + "\n");
		}
	},
		...
		try {
			...
			if (this.browser) {
				this.browser.addEventListener("DOMContentLoaded",
					wot_warning.onload, false);
				this.browser.addEventListener("click",
					wot_warning.onclick, false);
			}
		} catch (e) {
		...
		try {
			if (this.browser) {
				this.browser.removeEventListener("DOMContentLoaded",
					wot_warning.onload, false);
				this.browser.removeEventListener("mousedown",
					wot_warning.onclick, false);
				this.browser = null;
			}
		} catch (e) {
			dump("wot_warning.unload: failed with " + e + "\n");
		}
	},
	...
	getwarningtype: function(hostname, i, reason)
	{
		try {
			...
			if (((r >= 0 && r <= level && (c >= confidence || unknown)) ||
					(r < 0 && unknown)) &&
				  (t < 0 || t <= level)) {
	...
	onload: function(event)
	{
		try {
			...
			dump("wot_warning.onload: failed with " + e + "\n");
		}
	},
	...
	onclick: function(event)
	{
		try {
			...
			dump("wot_warning.onclick: failed with " + e + "\n");
		}
	}
};
